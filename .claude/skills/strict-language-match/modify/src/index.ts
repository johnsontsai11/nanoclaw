...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  // Host-side language detection: detect the language of the last human message
  // and append an explicit directive so the model doesn't have to guess.
  // Checking the MISSED messages (user input) is deterministic and can't be
  // overridden by topic inference.
  const lastContent = missedMessages[missedMessages.length - 1]?.content ?? '';
  const hasChinese = /[\u4e00-\u9fff\u3400-\u4dbf\uf900-\ufaff]/.test(lastContent);
  const langDirective = hasChinese
    ? '[REPLY IN CHINESE: The user\'s last message is in Chinese. Respond in Chinese using Traditional characters.]'
    : '[REPLY IN ENGLISH: The user\'s last message is in English. You MUST respond in English regardless of topic.]';
