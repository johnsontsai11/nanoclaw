      ...
      for (const msg of messages) {
        try {
          ...
          if (groups[chatJid]) {
            ...
            // Detect bot messages: if fromMe is true, it's definitely the bot
            // (or someone using the bot's account). Mark it as a bot message
            // to prevent infinite loops in groups where no trigger is required.
            const isBotMessage = fromMe && content.startsWith(`${ASSISTANT_NAME}:`);
